...
function concat_n(xforms) {
	var rv = identity()
	for (var ii = 0; ii < xforms.length; ii++)
		rv = concat(rv, xforms[ii]) 
...
function augment(shape1, shape2) {
	var s1p = shape1[0]
	var off = s1p.length
	for (var ii = 0; ii < shape2[0].length; ii++)
		s1p.push(shape2[0][ii])
...
angle = 0
var torus = make_torus(1, 3, 24, 24)
...
function draw_torus(canvas) {
	...
	if (ms) {
		...
		if (msvalue.length > 25)
			msvalue = msvalue.substr(msvalue.length - 25)
		ms.value = msvalue
	}
}
